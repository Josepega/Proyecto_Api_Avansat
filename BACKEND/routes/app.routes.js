...
router.delete("/borrar_cliente", (req, res) => {
  ...
  conexionMySQL.query(sql, [id_cliente], error => {
    if (error) {
      res.json({
        "status": 500,
        "mensaje": "<span class='error'>Error en el borrado del cliente. Error:" + error + "</span>"
      });
    } else {
      res.json({
        "status": 200,
        "mensaje": "<span class='correcto'>Cliente eliminado correctamente! <i class='fas fa-spinner fa-spin'></i></span>"
      });
    }
  });
});
